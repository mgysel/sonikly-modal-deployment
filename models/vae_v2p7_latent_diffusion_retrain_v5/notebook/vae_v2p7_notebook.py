# -*- coding: utf-8 -*-
"""V2.7 Conditional VAE - Latent Diffusion - ResMLP - Retrained - SOTA.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/12_7-5r3Hg7rgCxauDfH0OVgXAy0SiLgk
...
SAVE_DIR = "/content/drive/My Drive/Colab Notebooks/notebooks/models/vae/text_input/V2 - Activity Gating, FiLM/v2p7 - retrained/output_models"
...
class LatentDiffusionModel(tf.keras.Model):
    ...
    def generate(self, text_embeds, steps=50, guidance_scale=7.5):
        """
        Generates latents using Classifier-Free Guidance (CFG).
        ...
        # We use zeros to represent "no text"
        uncond_embeds = tf.zeros_like(text_embeds)

        # 3. Denoising Loop
        for i in reversed(range(0, self.timesteps)):
            # Create timestamp for this step
            t = tf.ones((batch_size,), dtype=tf.int32) * i

            # --- CFG TRICK: Batch Conditional and Unconditional together ---
            # We stack inputs to run them through the model in one pass
            ...
            # Split the predictions back apart
            noise_cond, noise_uncond = tf.split(noise_preds, num_or_size_splits=2, axis=0)

            # Apply Guidance Formula:
            # "Move away from the unconditioned result, towards the text result"
            ...
            # Calculate the "denoised" mean for this step
            model_mean = (1 / tf.sqrt(alpha)) * (z - ((1 - alpha) / (sqrt_one_minus_alpha_cumprod)) * pred_noise)

            # Add noise for the next step (unless it's the final step)
            if i > 0:
        ...
        # 4. Decode the final clean latent
...
def build_resmlp_denoiser(latent_dim=128, text_embed_dim=512, hidden_dim=1024, num_layers=16):
    ...
    # Condition Mapping (Shared across layers for efficiency)
...
# Cell 4.1 — Diffusion Inference Helpers (With CFG Scale)
...
def generate_synth_parameters_vae(text_description, model, parameter_types, categorical_num_classes,
                                  ...
                                  guidance_scale=7.5):  # <--- UPDATED: CFG Scale Arg
    if seed is not None:
        import torch
        np.random.seed(seed)
        random.seed(seed)
    ...
    print(f"Diffusion Sampling for: '{text_description}' (Scale: {guidance_scale})...")

    # !!! Using the new CFG-enabled generate method !!!
...
if 'best_model' in globals():
    generated_params = generate_synth_parameters_vae(
        ...
        seed=42,
        guidance_scale=7.5 # <--- Try 1.0 (Loose) vs 7.5 (Strict)
    )
