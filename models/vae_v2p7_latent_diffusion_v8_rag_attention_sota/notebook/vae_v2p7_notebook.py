# -*- coding: utf-8 -*-
"""V2.7 Conditional VAE - Latent Diffusion - ResMLP - Retrained - SOTA - RAG - Cross-Attention.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1styQZtdz0HkaZDcsBe0uaS9sr0bTwYBY
...
SAVE_DIR = "/content/drive/My Drive/Colab Notebooks/notebooks/models/vae/text_input/V2 - Activity Gating, FiLM/v2p7 - retrained - rag/output_models - cross_attention - b0p000001,128ldim"
...
# Cell 3.1 — Load, encode, and split data
...
# Cell 3.1.5 — Save Full RAG Database (X_full, Y_full)

import numpy as np
import os

# 1. Define file paths within your Save Directory
EMB_SAVE_PATH = os.path.join(SAVE_DIR, "rag_db_embeddings.npy")
PARAM_SAVE_PATH = os.path.join(SAVE_DIR, "rag_db_params.npy")

# 2. Prepare the data
# description_embeddings (X) and serum_params (Y) represent the whole filtered dataset
X_full = description_embeddings.astype('float32')
Y_full = serum_params.astype('float32')

print(f"--- Exporting RAG Knowledge Base ---")
print(f"Total Presets: {len(Y_full)}")
print(f"Embeddings:    {X_full.shape}")
print(f"Parameters:    {Y_full.shape}")

# 3. Save to Google Drive
try:
    np.save(EMB_SAVE_PATH, X_full)
    np.save(PARAM_SAVE_PATH, Y_full)
    print(f"✅ RAG Database successfully saved to Drive.")
    print(f"📍 Path (X): {EMB_SAVE_PATH}")
    print(f"📍 Path (Y): {PARAM_SAVE_PATH}")
except Exception as e:
...
class LatentDiffusionModel(tf.keras.Model):
    """ Stage 2: Latent Diffusion Model (Updated with CFG, RAG, and Min-SNR) """
    ...
    def train_step(self, data):
        ...
        # --- MIN-SNR LOSS WEIGHTING (FIXED SHAPES) ---
        # Get alpha_cumprod for the current timestep t (Shape: [batch_size])
        alpha_cumprod = tf.gather(self.scheduler.alphas_cumprod, t)

        # Reshape to [batch_size, 1] for broadcasting against noise [batch_size, 64]
        alpha_cumprod = tf.reshape(alpha_cumprod, [batch_size, 1])

        # Calculate SNR: alpha_bar / (1 - alpha_bar)
        snr = alpha_cumprod / (1.0 - alpha_cumprod)

        # Weighting strategy: Clamp max weight to 5.0
        loss_weight = tf.minimum(snr, 5.0) / snr
        # ---------------------------------------------
        ...
        with tf.GradientTape() as tape:
            ...
            # Apply Min-SNR Weighting to the squared error
            # Error is (noise - pred_noise)^2 -> Shape [batch_size, 64]
            # loss_weight is [batch_size, 1] -> Broadcasts correctly
            loss = tf.reduce_mean(loss_weight * tf.square(noise - pred_noise))
    ...
    def generate(self, text_embeds, steps=50, guidance_scale=7.5, initial_latent=None, strength=0.8):
        ...
        inference_indices = np.linspace(0, self.timesteps - 1, steps, dtype=int)

        # 2. Determine Start Point
        if initial_latent is None:
            start_index = len(inference_indices) - 1
            z = tf.random.normal(shape=(batch_size, latent_dim))
        else:
            target_timestep = int(self.timesteps * strength)
            start_index = (np.abs(inference_indices - target_timestep)).argmin()
            t_start_val = inference_indices[start_index]

            # Fix broadcasting for inference noise addition
            noise = tf.random.normal(shape=tf.shape(initial_latent))
            t_tensor = tf.ones((batch_size,), dtype=tf.int32) * t_start_val
            z = self.scheduler.add_noise(initial_latent, noise, t_tensor)

        uncond_embeds = tf.zeros_like(text_embeds)

        # 3. Denoising Loop
        for i in range(start_index, -1, -1):
            t_val = inference_indices[i]
            t = tf.ones((batch_size,), dtype=tf.int32) * t_val
            ...
            # Gather scalar values for this step
            alpha = tf.gather(self.scheduler.alphas, t_val)
            alpha_cumprod = tf.gather(self.scheduler.alphas_cumprod, t_val)
            beta = tf.gather(self.scheduler.betas, t_val)

            # Reshape for broadcasting
            # (Note: alpha is usually scalar, but just in case)
            alpha = tf.reshape(alpha, [1, 1])
            alpha_cumprod = tf.reshape(alpha_cumprod, [1, 1])
...
# Cell 3.3 — Model Builders (SOTA: Hybrid Self + Cross Attention)

from tensorflow.keras.regularizers import l2
from tensorflow.keras.layers import MultiHeadAttention, LayerNormalization, Add, Reshape, Flatten

# (Keep build_encoder and build_decoder_film exactly as they are...)
...
def build_resmlp_denoiser(latent_dim=128, text_embed_dim=512, hidden_dim=1024, num_layers=16):
    ...
    return Model(inputs=[z_noisy, t_in, text_emb], outputs=noise_pred, name="hybrid_attn_denoiser")
...
batch_size = 64
dropout_rate = 0.2
beta_max = 0.000001       # Low Beta for High Fidelity
...
def retrieve_closest_preset(target_emb, db_embeddings, db_params, top_k=10):
    """
    ...
    # 2. Get indices of the top_k highest scores
    # argsort sorts ascending, so we take the last k elements
    top_indices = np.argsort(scores)[-top_k:]

    # 3. Randomly sample one index from the top candidates
    best_idx = np.random.choice(top_indices)

    print(f"🔎 RAG Retrieval: Sampled match from Top-{top_k} candidates at index {best_idx} (Score: {scores[best_idx]:.4f})")
