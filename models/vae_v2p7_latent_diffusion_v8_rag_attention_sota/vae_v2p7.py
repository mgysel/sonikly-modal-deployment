...
from keras.layers import Layer, Dense, LayerNormalization, Dropout, Add, Concatenate, LeakyReLU, Activation, MultiHeadAttention, Reshape, Flatten, Multiply
...
class LatentDiffusionModel(tf.keras.Model):
    ...
    def generate(self, text_embeds, steps=50, guidance_scale=7.5, initial_latent=None, strength=0.8):
        ...
        if initial_latent is None:
            ...
            # RAG: Noisy Reference Logic (Python loop for now as custom start index logic is tricky in JIT)
            # Or adapt the compiled loop. For simplicity + safety, let's use a Python loop for RAG first.
...
class VAE_V2P7:
    ...
    def load_rag_data(self, embeddings_path, params_path):
        """Load RAG database from separate files."""
        if self.rag_data is not None:
             return
        
        if not os.path.exists(embeddings_path) or not os.path.exists(params_path):
            print(f"⚠️ RAG data missing. Checked: {embeddings_path} and {params_path}")
            return

        try:
            print(f"Loading RAG data...")
            X_train = np.load(embeddings_path, allow_pickle=True)
            y_train = np.load(params_path, allow_pickle=True)
            
            self.rag_data = {
                'X_train': X_train,
                'y_train': y_train
            }
            print(f"✅ RAG data loaded. {len(X_train)} presets available.")
        except Exception as e:
            print(f"❌ Error loading RAG data: {e}")

    def retrieve_closest_preset(self, target_emb, top_k=10):
        """Finds a random preset from the top_k closest matches to ensure variety."""
        if self.rag_data is None: return None
        
        db_embeddings = self.rag_data['X_train']
        db_params = self.rag_data['y_train']
        
        # Dot product (embeddings should be normalized from CLAP)
        # target_emb shape: (1, 512) or (512,)
        # db_embeddings shape: (N, 512)
        ...
        # Randomly sample one index from the top candidates
        best_idx = np.random.choice(top_indices)
        
        print(f"🔎 RAG Retrieval: Sampled match from Top-{k} candidates at index {best_idx} (Score: {scores[best_idx]:.4f})")
    ...
    def generate(self, text_description, diffusion_steps=None, head_noise_level=0.0,
                 sample_categorical=True, seed=None, guidance_scale=7.5, 
                 use_rag=False, rag_strength=0.8, rag_top_k=10, verbose=True):
        """Generate synthesizer parameters from text description using latent diffusion.
        
        Args:
            text_description: Text prompt (string) or list of text prompts
            diffusion_steps: Number of denoising steps
            head_noise_level: Noise level to add to output heads
            sample_categorical: If True, sample from categorical distributions
            seed: Random seed
            guidance_scale: CFG guidance scale
            use_rag: If True, uses RAG database to find closest preset as starting point
            rag_strength: 0.0 to 1.0. Higher = more noise added to retrieved latent (0.8 = 80% noise)
            rag_top_k: Number of top candidates to sample from for RAG
            verbose: Print debug info
        
        Returns:
            np.ndarray: Parameter array of shape (202,) or (batch_size, 202)
        """
        if seed is not None:
            np.random.seed(seed)
            import random
        ...
        # --- RAG RETRIEVAL & ENCODING ---
        initial_latent = None
        if use_rag and self.rag_data is not None:
            try:
                if verbose: print(f"🔎 Performing RAG Retrieval (Top-{rag_top_k})...")
                # Vectorized Cosine Similarity
                # emb: (B, 512), db: (N, 512) -> scores: (B, N)
                # Assumes embeddings are normalized (CLAP output usually is)
                scores = np.dot(emb, self.rag_data['X_train'].T)
                
                # Sample from top k for each item in batch
                best_indices = []
                for i in range(batch_size):
                    sample_scores = scores[i]
                    k = min(rag_top_k, len(sample_scores))
                    # Top k indices
                    top_k_indices = np.argpartition(sample_scores, -k)[-k:]
                    # Randomly choose one
                ...
                if verbose:
                    print(f"   Matches: {best_indices} (Scores: {scores[np.arange(batch_size), best_indices]})")
                
                # Retrieve Params (B, 202)
                retrieved_params = self.rag_data['y_train'][best_indices]
                
                # Encode to Latent Space (B, 128)
                # Note: We use the *query* text embedding + *retrieved* params
                # This conditions the initial latent on the logical connection between the text and the gathered preset
                z_mean, _ = ldm_model.vae_encoder([emb, retrieved_params], training=False)
                initial_latent = z_mean
                
            except Exception as e:
                print(f"⚠️ RAG Verification Failed: {e}")
                import traceback
                traceback.print_exc()
                initial_latent = None # Fallback to standard generation

        try:
            # Run diffusion generation
            steps_to_use = diffusion_steps if diffusion_steps is not None else ldm_model.timesteps
            if verbose:
                print(f"Running {steps_to_use}-step diffusion (RAG={bool(initial_latent)})...")
            
            # Call LDM Generate (Updated signature)
            predicted_outputs = ldm_model.generate(
                emb, 
                steps=steps_to_use, 
                guidance_scale=guidance_scale,
                initial_latent=initial_latent,
                strength=rag_strength
            )
    ...
    def __repr__(self):
        return f"VAE_V2P7(model_path={self.model_path}, embedding={self.embedding_model_type}, steps={self.default_diffusion_steps}, rag={self.rag_data is not None})"

# --- UPDATED: Denoiser with Self + Cross Attention ---
def build_resmlp_denoiser(latent_dim=128, text_embed_dim=512, hidden_dim=1024, num_layers=16):
    z_noisy = keras.Input(shape=(latent_dim,), name="z_noisy")
    t_in = keras.Input(shape=(1,), name="time_input")
    text_emb = keras.Input(shape=(text_embed_dim,), name="text_embedding")
    ...
    # --- BLOCK 1: ResNet Layers (0-7) ---
    for i in range(num_layers // 2):
        x = ResidualBlock(width=hidden_dim, name=f"res_block_{i}")(x, cond_mapped)

    # --- BLOCK 2: ATTENTION CENTER ---
    # 1. Reshape for Attention: (Batch, 16, 64)
    # 1024 / 16 = 64
    seq_len = 16
    token_dim = hidden_dim // seq_len
    x_seq = Reshape((seq_len, token_dim))(x)

    # 2. Self-Attention (Internal Logic)
    attn_out = MultiHeadAttention(num_heads=8, key_dim=64)(x_seq, x_seq)
    x_seq = Add()([x_seq, attn_out])
    x_seq = LayerNormalization()(x_seq)

    # 3. Cross-Attention (Text Control)
    # Prepare text: (Batch, 512) -> (Batch, 1, 512)
    context = Reshape((1, text_embed_dim))(text_emb)

    # "Query" is the latent image, "Key/Value" is the text
    cross_out = MultiHeadAttention(num_heads=8, key_dim=64)(query=x_seq, value=context, key=context)
    x_seq = Add()([x_seq, cross_out])
    x_seq = LayerNormalization()(x_seq)

    # 4. Flatten back to vector
    x_att = Flatten()(x_seq)
    x = Add()([x, x_att])

    # --- BLOCK 3: ResNet Layers (8-15) ---
    ...
    return keras.Model(inputs=[z_noisy, t_in, text_emb], outputs=noise_pred, name="hybrid_attn_denoiser")
