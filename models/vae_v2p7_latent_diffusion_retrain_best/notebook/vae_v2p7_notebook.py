# -*- coding: utf-8 -*-
"""BEST V2.7 Conditional VAE - Latent Diffusion - ResMLP - Retrained.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1fjczlA-5fs_09szaHEkOXv_VL377DNpi
...
class VAE_Text_to_Synth_Standard(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            # Loss will now sanitize y_true internally
...
# Cell 3.4 — Hyperparameter Sweep (6 Strategic Configs)
...
# --- Visual Setup for Pandas ---
pd.set_option('display.max_columns', None)
pd.set_option('display.width', 1000)

# Define 6 targeted configurations
configs = [
    {"name": "A: Tight & Smart",   "latent": 32,  "width": 512,  "beta": 0.1},
    {"name": "B: Balanced Std",    "latent": 64,  "width": 512,  "beta": 0.1},
    {"name": "C: High Res",        "latent": 96,  "width": 1024, "beta": 0.2},
    {"name": "D: Max Capacity",    "latent": 128, "width": 1024, "beta": 0.5},
    {"name": "E: High Reg",        "latent": 64,  "width": 1024, "beta": 1.0},
    {"name": "F: Wide & Loose",    "latent": 128, "width": 2048, "beta": 0.1},
]

results = []

print(f"=== 🧪 STARTING 6-WAY STRATEGIC SWEEP (12 Epochs each) ===")

for cfg in configs:
    print(f"\n--- Testing {cfg['name']} ---")
    print(f"    [Latent: {cfg['latent']} | Width: {cfg['width']} | Beta: {cfg['beta']}]")

    # 1. Clear Memory
    tf.keras.backend.clear_session()

    # 2. Build Model
    enc = build_encoder(X_train.shape[1], num_params, cfg['latent'], cfg['width'], dropout_rate=0.3)
    dec = build_decoder_film(X_train.shape[1], cfg['latent'], cfg['width'],
    ...
    # 4. Log Results (Average last 3 epochs for stability)
    val_loss = np.mean(hist.history['val_loss'][-3:])
    val_recon = np.mean(hist.history['val_recon'][-3:])
    val_kl = np.mean(hist.history['val_kl'][-3:])

    print(f"    > Result: Val Loss: {val_loss:.2f} | Recon: {val_recon:.2f} | KL: {val_kl:.2f}")
...
print("\n" + "="*40)
print("🏆 FINAL SWEEP RESULTS (Sorted by Val Loss)")
print("="*40)
df = pd.DataFrame(results)
# Sort by Validation Loss (Lower is better)
df_sorted = df.sort_values(by="Val Loss")
...
print(f"   Use these settings for final training:")
print(f"   Latent Dim: {int(best['Latent'])}")
print(f"   Dec Width:  {int(best['Width'])}")
print(f"   Beta:       {best['Beta']}")
print("="*40)

# Cell 3.4 — Two-Stage Training Loop (Final Configuration)
...
# --- CONFIGURATION (Based on Sweep Winner: Config A) ---
latent_dim = 32        # The Sweep Winner
vae_epochs = 60        # Full training duration
diffusion_epochs = 60
lr = 1e-3
batch_size = 128       # Keep the stable batch size

# --- STAGE 1: TRAIN VAE (Compression) ---
print(f"\n=== STAGE 1: Training VAE (Latent {latent_dim} | Width 512 | Beta 0.1) ===")

tf.keras.backend.clear_session()

# Build Model with Winning Settings (Width 512, Dropout 0.3)
enc = build_encoder(X_train.shape[1], num_params, latent_dim, 512, dropout_rate=0.3)
dec = build_decoder_film(X_train.shape[1], latent_dim, 512,
                         unipolar_indices, bipolar_indices, boolean_param_indices,
                         categorical_param_indices, categorical_num_classes,
                         dropout_rate=0.3)

# Initialize VAE with Beta 0.1
vae = VAE_Text_to_Synth_Standard(
    encoder=enc, decoder=dec,
    unipolar_indices=unipolar_indices, bipolar_indices=bipolar_indices,
    bool_indices=boolean_param_indices, cat_indices=categorical_param_indices,
...
callbacks_vae = [
    # Give it plenty of patience since we are training for long duration
...
# --- STAGE 2: TRAIN FLOW MATCHING (Generation) ---
...
# Build Denoiser (Automatically adjusts to latent_dim=32)
