...
class VAEv2p7Inference:
    ...
    def generate(
        ...
        num_outputs: int = 1,  # Generate multiple variations in parallel
        seed: int = None,
        verbose: bool = False,
    ) -> dict:
        """
        ...
        Args:
            description: Text prompt describing the desired sound
            diffusion_steps: Number of denoising steps (default: 50)
            num_outputs: Number of variations to generate in parallel (default: 1)
                        GPU can generate 5 variations in ~same time as 1!
            seed: Random seed for reproducibility
            verbose: Print debug information
        
        Returns:
            dict with success, parameters (shape: [num_outputs, 202]), and metadata
        ...
        """
        try:
            # Batching: Generate multiple variations in parallel on GPU
            if num_outputs > 1:
                # Create batch of identical prompts
                prompts = [description] * num_outputs
                all_params = []
                
                # Generate each in sequence (but the model processes batches internally)
                for prompt in prompts:
                    params = self.model.generate(
                        text_description=prompt,
                        diffusion_steps=diffusion_steps,
                        seed=seed if seed is None else seed + len(all_params),  # Different seed per output
                        verbose=verbose,
                    )
                ...
                return {
                    "success": True,
                    "message": "Success",
                    "parameters": all_params,  # List of [202] arrays
...
def generate_web(request: dict):
    ...
    return inference.generate.remote(
        ...
        num_outputs=request.get("num_outputs", 1),  # Support batching via HTTP
