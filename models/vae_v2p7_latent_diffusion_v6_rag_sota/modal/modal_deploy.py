...
# Conditionally add RAG data if it exists locally
if local_rag_embeddings_path.exists() and local_rag_params_path.exists():
    image = (
        image
...
class VAEv2p7Inference:
    ...
    @modal.method()
    def generate(self, description: str, diffusion_steps: int = 50, num_outputs: int = 1, seed: int = None, 
                 guidance_scale: float = 7.5, use_rag: bool = False, rag_strength: float = 0.8, verbose: bool = False):
        """
        ...
        try:
            ...
            if num_outputs > 1:
                # If seed is None, generate a random base seed effectively
                if seed is None:
                    import time
