# -*- coding: utf-8 -*-
"""V2.7 Conditional VAE - Flow - ResMLP.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1zy5iInu7NlwF2SMFxWqvJNkVNeVQ3_47
...
SAVE_DIR = "/content/drive/My Drive/Colab Notebooks/notebooks/models/vae/text_input/V2 - Activity Gating, FiLM/v2p7 - retrained - flow/output_models"
...
W_MOD_GATE  = 5.0    # Weight for modulation slots (KEPT ORIGINAL AS REQUESTED)
...
class VAE_Text_to_Synth_Standard(tf.keras.Model):
    ...
    def __init__(self, encoder, decoder, unipolar_indices, bipolar_indices,
                 ...
                 latent_dim, beta=0.2, latent_dropout_rate=0.0, **kwargs):
        super().__init__(**kwargs)
...
# ==============================================================================
# 3. FLOW MATCHING COMPONENTS (Stage 2: Rectified Flow)
# ==============================================================================
...
class LatentDiffusionModel(tf.keras.Model):
    ...
    def generate(self, text_embeds, steps=25, cfg_scale=4.0):
        ...
        # 2. Setup Solver
        dt = 1.0 / steps
        null_embeds = tf.zeros_like(text_embeds) # Unconditional input

        # 3. Euler Loop
        for i in range(steps):
            t_val = i / steps
            t_tensor = tf.ones((batch_size, 1)) * (t_val * 1000.0) # Scale for embedding

            # Predict Velocity for Conditional (Text) and Unconditional (Null)
            v_cond = self.denoiser([z, t_tensor, text_embeds], training=False)
            v_uncond = self.denoiser([z, t_tensor, null_embeds], training=False)

            # CFG Step: Push away from generic, towards prompt
...
# --- CONFIG ---
latent_dim = 128      # <--- RESTORED to 128 (Essential for fidelity)
vae_epochs = 40
diffusion_epochs = 60
lr = 1e-3
batch_size = 64
...
# Build VAE
# Increased Encoder to 512, Decoder to 1024 to capture complex Mod Matrix rules
enc = build_encoder(X_train.shape[1], num_params, latent_dim, 512)
dec = build_decoder_film(X_train.shape[1], latent_dim, 1024, # <--- INCREASED CAPACITY
...
callbacks_vae = [
    ...
    # LOW BETA = SHARPER PRESETS
    BetaAnnealing(max_beta=0.1, warmup_epochs=10) # <--- SET TO 0.1 for maximum detail
]
...
print("\n=== STAGE 2: Training Flow Matching (Generation) ===")
...
# Initialize Flow Matching Model (No timesteps arg needed anymore)
ldm = LatentDiffusionModel(vae_encoder=vae.encoder, vae_decoder=vae.decoder, denoiser=denoiser)
...
# Cell 3.5 — Evaluate Diffusion Model (Corrected for Flow Matching)
...
print("Evaluating Diffusion Model on Validation Set (Flow Matching)...")

for (text_batch, params_batch), _ in val_dataset:
    # 1. Encode Data to get z_1 (Target Latent)
    # We use the internal VAE encoder to get the ground truth latent for this text/preset
    z_mean, z_log_var = best_model.vae_encoder([text_batch, params_batch], training=False)
    z_1 = z_mean  # Use mean for validation stability

    # 2. Sample Noise z_0 (Random Gaussian)
    z_0 = tf.random.normal(shape=tf.shape(z_1))

    # 3. Sample Continuous Time t [0.0 to 1.0]
    batch_size = tf.shape(z_0)[0]
    t = tf.random.uniform((batch_size, 1), minval=0.0, maxval=1.0)

    # 4. Interpolate (Rectified Flow Formula)
    # The model learns to predict the path: z_t = (1 - t)*Noise + t*Data
    z_t = (1.0 - t) * z_0 + t * z_1

    # 5. Calculate Target Velocity (The Vector direction)
    # Target = Data - Noise
    target_v = z_1 - z_0

    # 6. Predict Velocity using the Denoiser
    # IMPORTANT: We must scale 't' by 1000.0 because that is how the Sinusoidal Embedding
    # was trained in your LatentDiffusionModel class.
    pred_v = best_model.denoiser([z_t, t * 1000.0, text_batch], training=False)

    # 7. Calculate MSE (Velocity Prediction Error)
    loss = tf.reduce_mean(tf.square(target_v - pred_v))
    total_mse_agg.update_state(loss)

print("-" * 30)
print(f"Validation Flow Matching MSE: {total_mse_agg.result():.4f}")
print("-" * 30)
...
def interpolate_prompts(model, start_prompt, end_prompt, steps=10, z_seed=41):
    """
