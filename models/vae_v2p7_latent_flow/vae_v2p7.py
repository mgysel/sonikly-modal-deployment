...
# -------------------------------------------------------------------------
# Latent Diffusion Model (Stage 2: Generation - Flow Matching)
# -------------------------------------------------------------------------
...
class LatentDiffusionModel(tf.keras.Model):
    """
    Stage 2: Flow Matching (Rectified Flow) with Classifier-Free Guidance (CFG)
    """
    def __init__(self, vae_encoder, vae_decoder, denoiser, **kwargs):
        super().__init__(**kwargs)
        self.vae_encoder = vae_encoder
        self.vae_decoder = vae_decoder
        self.denoiser = denoiser
        self.loss_tracker = tf.keras.metrics.Mean(name="loss")
        self.uncond_prob = 0.2  # 20% chance to drop text during training for CFG
    ...
    def train_step(self, data):
        ...
        # [IMPROVEMENT] Latent Mixup / Data Noise
        # Add slight noise to the target latent to smooth the manifold for small datasets
        jitter = tf.random.normal(tf.shape(z_mean), stddev=0.05)
        z_1 = z_mean + tf.exp(0.5 * z_log_var) * tf.random.normal(tf.shape(z_mean)) + jitter

        # 2. Sample Noise (z_0)
        z_0 = tf.random.normal(shape=tf.shape(z_1))

        # 3. Sample Time (t) from 0 to 1
        t = tf.random.uniform((batch_size, 1), minval=0.0, maxval=1.0)

        # 4. Interpolate (Straight Line Rectified Flow)
        # Formula: z_t = (1 - t) * z_0 + t * z_1
        z_t = (1.0 - t) * z_0 + t * z_1

        # 5. Target Velocity (v)
        # The vector pointing straight from Noise to Data
        target_v = z_1 - z_0

        # 6. CFG: Randomly Mask Text
        # Create mask: 1 = keep text, 0 = drop text (empty condition)
        rand_mask = tf.cast(tf.random.uniform((batch_size, 1)) > self.uncond_prob, tf.float32)
        text_embeds_forcing = text_embeds * rand_mask

        # 7. Train Denoiser to predict Velocity
        with tf.GradientTape() as tape:
            # We scale t by 1000 for the embedding layer to work effectively
            pred_v = self.denoiser([z_t, t * 1000.0, text_embeds_forcing], training=True)
            loss = tf.reduce_mean(tf.square(target_v - pred_v))

        grads = tape.gradient(loss, self.denoiser.trainable_variables)
        self.optimizer.apply_gradients(zip(grads, self.denoiser.trainable_variables))
    ...
    def test_step(self, data):
        (text_embeds, params_in), _ = data
        z_mean, z_log_var = self.vae_encoder([text_embeds, params_in], training=False)
        z_1 = z_mean # No jitter for validation
        z_0 = tf.random.normal(shape=tf.shape(z_1))
        t = tf.random.uniform((tf.shape(z_1)[0], 1), minval=0.0, maxval=1.0)
        z_t = (1.0 - t) * z_0 + t * z_1
        target_v = z_1 - z_0
        pred_v = self.denoiser([z_t, t * 1000.0, text_embeds], training=False)
        loss = tf.reduce_mean(tf.square(target_v - pred_v))
        self.loss_tracker.update_state(loss)
        return {"loss": self.loss_tracker.result()}

    @tf.function(jit_compile=False)
    def _flow_matching_loop_compiled(self, z, text_embeds, null_embeds, steps, cfg_scale):
        """JIT-compiled Euler solver loop"""
        if steps == 0: return z # Sanity check
        
        # Note: tf.print works inside tf.function
        tf.print("🌊 Flow Matching Steps:", steps, "| CFG:", cfg_scale)
        
        batch_size = tf.shape(z)[0]
        dt = 1.0 / tf.cast(steps, tf.float32)

        for i in tf.range(steps):
            t_val = tf.cast(i, tf.float32) / tf.cast(steps, tf.float32)
            t_tensor = tf.ones((batch_size, 1)) * (t_val * 1000.0)

            # Predict Velocity
            v_cond = self.denoiser([z, t_tensor, text_embeds], training=False)
            v_uncond = self.denoiser([z, t_tensor, null_embeds], training=False)
    ...
    def generate(self, text_embeds, steps=25, cfg_scale=4.0):
        """ Inference using Euler ODE Solver + CFG """
        ...
        # 3. Run Compiled Loop
        z = self._flow_matching_loop_compiled(z, text_embeds, null_embeds, tf.constant(steps), tf.constant(cfg_scale, dtype=tf.float32))
...
class VAE_V2P7:
    ...
    def generate(self, text_description, diffusion_steps=None, cfg_scale=4.0, head_noise_level=0.0,
                 sample_categorical=True, seed=None, verbose=True):
        ...
        Args:
            ...
            diffusion_steps: Number of denoising steps (default: use model's default, typically 25-50)
        ...
        try:
            # Run diffusion generation (Flow Matching solver)
            if verbose:
                steps_to_use = diffusion_steps if diffusion_steps is not None else self.default_diffusion_steps
                print(f"Running {steps_to_use}-step Flow Matching for batch_size={batch_size}, CFG={cfg_scale}...")
            
            predicted_outputs = ldm_model.generate(
                emb, 
                steps=diffusion_steps or self.default_diffusion_steps,
                cfg_scale=cfg_scale
            )
